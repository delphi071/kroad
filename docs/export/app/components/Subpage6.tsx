      ...
      {items.map((it) => {
        ...
        return active ? (
          ...
          <button
            ...
            style={{ left: it.left, top: F(770.38), fontSize: lang === "en" ? 40 : 54 }}
          >
            {mainLabel}
          </button>
        );
      ...
      {items.map((it) => {
        ...
        return active ? (
          ...
          >
            ...
            <p className="font-pretendard font-bold leading-normal tracking-[-0.96px] whitespace-nowrap" style={{ fontSize: lang === "en" ? 18 : 24 }}>{mainLabel}</p>
          </button>
        ) : (
          <button
            key={it.id}
      ...
      <div className="absolute flex flex-col gap-[46px] items-start justify-center py-[40px]" style={{ left: 200, top: F(1460), width: 1520 }}>
        <div className="flex flex-col gap-[8px] items-start">
          <div className="flex gap-[42px] items-center">
            <p className="font-pretendard font-bold leading-[1.2] text-grayscale-900 tracking-[-1.3px] whitespace-nowrap" style={{ fontSize: 44 }}>{lang === "en" ? "NH Bank " : "농협 "}301-0061-8049-01</p>
            <button
              type="button"
              onClick={handleCopy}
          ...
          <p className={`font-pretendard leading-[1.1] text-grayscale-900 tracking-[-0.36px] ${lang === "en" ? "" : "whitespace-nowrap"}`} style={{ fontSize: 32, fontWeight: 800 }}>{t.subpage6.donate.bankAccountName}</p>
        </div>
        <p className="font-pretendard leading-[1.3] text-primary tracking-[-0.72px]" style={{ fontSize: 32, width: 600 }}>
          {t.subpage6.donate.bankNote}
        </p>
      </div>
      {/* 후원금 사용처 heading */}
      <p className="absolute font-pretendard leading-[1.1] text-grayscale-900 whitespace-nowrap" style={{ left: 198, top: F(1989), fontSize: 32, fontWeight: 800, letterSpacing: "-0.36px" }}>
      ...
      <div className="absolute flex flex-col items-start whitespace-nowrap" style={{ left: 198, top: F(954), width: 1520 }}>
        {REPORTS.map((r) => (
          <button
            ...
            style={{ height: 121 }}
          >
            <p className="font-montserrat font-bold leading-none text-grayscale-600 shrink-0" style={{ fontSize: 18 }}>{r.date}</p>
            <p className="font-pretendard font-bold leading-[1.3] text-grayscale-900 tracking-[-0.72px]" style={{ fontSize: 32 }}>{r.title}</p>
          </button>
        ))}
      </div>
      {/* Pagination */}
  ...
  return (
    <>
      ...
      >
        ...
        <span className="font-pretendard leading-[1.3] text-[#9c9c9c] tracking-[-0.24px] whitespace-nowrap" style={{ fontSize: 20 }}>목록</span>
      </button>
      <div
      ...
      >
        <p className="font-pretendard font-bold leading-[1.3] text-grayscale-900 tracking-[-1.56px]" style={{ fontSize: 54 }}>{r.title}</p>
        <p className="font-montserrat font-bold leading-none text-grayscale-600" style={{ fontSize: 18 }}>{r.date}</p>
      </div>
      <div
      ...
      <div className="flex flex-col items-center justify-center w-full">
        <div className="flex flex-col gap-[8px] items-center">
          <div className="flex gap-[14px] items-center">
            <p className="font-pretendard font-bold leading-normal text-grayscale-900 tracking-[-0.96px] whitespace-nowrap" style={{ fontSize: 24 }}>{lang === "en" ? "NH Bank " : "농협 "}301-0061-8049-01</p>
            <button
              type="button"
              onClick={handleCopy}
        ...
        <div className="flex items-center justify-center">
          <p className="font-pretendard leading-normal text-primary tracking-[-0.8px] text-center" style={{ fontSize: 16 }}>{t.subpage6.donate.bankNote}</p>
        </div>
      </div>
      <div className="flex flex-col gap-[50px] items-center w-full">
        <p className="font-pretendard font-bold leading-normal text-grayscale-900 tracking-[-0.8px] text-center" style={{ fontSize: 16 }}>{t.subpage6.donate.circlesLabel}</p>
            ...
            return (
              <div key={idx} className="aspect-[1/1] overflow-clip relative rounded-full w-full">
                ...
                <div className="absolute left-1/2 -translate-x-1/2 font-pretendard font-bold text-white text-center tracking-[-0.96px]" style={{ top: "calc(50% - 36px)", fontSize: lang === "en" ? 18 : 24, lineHeight: 1.5, width: "85%" }}>
                  {lines.map((line, i) => (
                    <p key={i} className="leading-normal" style={{ marginBottom: 0 }}>{line}</p>
                  ))}
                </div>
              </div>
            );
      ...
      <div className="flex flex-col items-start font-pretendard font-bold w-screen -mx-[20px]">
        {REPORTS.map((r) => (
          ...
          >
            ...
            <p className="leading-normal text-grayscale-900 tracking-[-0.8px]" style={{ fontSize: 16, fontWeight: 700 }}>{r.title}</p>
          </button>
        ))}
      </div>
      <div className="flex items-center justify-between w-full">
...
function MobileReportDetail({ reportId, onBack }: { reportId: number; onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col items-start pb-[39px] pt-[15px] w-screen -mx-[20px]">
      ...
      >
        <span className="block size-[24px]"><ArrowLeft size={24} /></span>
        <span className="font-pretendard font-bold leading-normal text-[#9c9c9c] tracking-[-0.8px]" style={{ fontSize: 16 }}>목록</span>
      </button>
