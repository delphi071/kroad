...
import { getMainHref, getSubHref, goToFirstScreen } from "./navLinks";
...
const COLLAPSED_H_DESKTOP = 280; // 1920-frame baseline
...
export default function Hero1Section() {
  ...
  // Expand (un-collapse) the hero whenever navigation targets the plain #h1
  // (e.g. the logo link). Without this the React `collapsed` state would stay
  // true and the user would land on the collapsed bar + Subpage1 content
  // instead of the full hero section.
  useEffect(() => {
    const handler = () => {
      const hash = window.location.hash;
      if (hash === "#h1" || hash === "") expand();
    };
  ...
  /* Mobile: always relative h-screen — no sticky/collapse.
     Desktop collapsed: sticky top-0 with compact 280/1920 ratio height. */
    ...
    <>
      ...
      >
        {/* Logo rendered as a CSS mask so its color can toggle with scroll position.
            ...
            Links back to the first slide (#h1) — the first screen after the splash. */}
        <a
          href="#h1"
          aria-label="홈으로"
          className="block cursor-pointer"
          onClick={(e) => { e.preventDefault(); goToFirstScreen(); }}
        >
          <div
      ...
      >
        ...
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="/figma/hero-scroll-chevron.png" alt="" aria-hidden className="block h-[26px] w-[26px]" />
          <span className="font-montserrat whitespace-nowrap text-[10px] font-bold leading-normal">
            SCROLL TO DISCOVER
          </span>
        </button>
      </div>
      ...
      <div className="hidden lg:block">
        ...
        >
          ...
          >
            ...
            >
              <a
                href="#h1"
                aria-label="홈으로"
                className="absolute left-[86px] h-[54.628px] w-[130px] cursor-pointer"
                style={{ top: collapsed ? 12 : 37.19 }}
                onClick={(e) => { e.preventDefault(); goToFirstScreen(); }}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
            ...
            {NAV_MENU.map((item, idx) => {
              ...
              const colorClass =
                idx === ACTIVE_INDEX ? "text-primary" : "text-grayscale-100";
              const labelText = (dictItem.label || `${item.line1 ?? ""} ${item.line2 ?? ""}`.trim() || item.label || "").replace(/\n/g, " ");
              const labelLines = (idx === 1 && lang === "en") ? labelText.split(", ") : [labelText];
              const labelTop = (idx === 1 && lang === "en") ? (collapsed ? 12 : 75) : (collapsed ? 20 : 96);
              return (
                <div
                  key={idx}
                ...
                >
                  <a
                    ...
                    onClick={() => { setMenuOpen(false); setNavHovered(false); window.dispatchEvent(new CustomEvent("mobile-nav-start")); }}
                    className={`font-pretendard absolute right-[18px] whitespace-nowrap text-right text-[13px] font-extrabold leading-[1.3] tracking-[-0.32px] ${colorClass}`}
                    style={{ top: labelTop }}
                  >
                    {labelLines.map((line, i) => <p key={i}>{line}</p>)}
                  </a>

                  <div
                    className="absolute right-[18px] flex flex-col items-end gap-[12px] transition-opacity duration-200"
                    style={{
                      top: collapsed ? 80 : 158,
          ...
          {/* Collapsed X — sibling of <nav> so hovering it doesn't expand the nav */}
          {collapsed && (
            <button
              type="button"
              aria-label="히어로 닫기"
              ...
              style={{ left: 1810, top: 180, fontSize: 40, lineHeight: 1 }}
            >
              ✕
            </button>
          )}
        ...
        {!collapsed && (
          ...
          >
            ...
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src="/figma/hero-scroll-chevron.png"
              alt=""
              aria-hidden
              className="absolute"
              style={{
                ...
                width: "calc(100% * 100 / 516)",
                height: "calc(100% * 100 / 244)",
              }}
            />
            <span
