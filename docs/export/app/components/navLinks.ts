...
/* Navigate to the first screen — slide 1's hero section.
   HeroCarousel only reacts to the `hashchange` event, so clicking a link to
   `#h1` does nothing when the hash is already `#h1` (e.g. the user scrolled
   down into slide 1's content). In that case we re-dispatch `hashchange`
   manually; otherwise we just set the hash. Either way HeroCarousel's
   scrollToHash resets the slide's scrollTop, so this always lands on the
   hero section, never the content below it. */
export function goToFirstScreen(): void {
